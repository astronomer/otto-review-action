...
- `action.yaml` — the composite action: inputs, outputs, and the ordered steps.
- `scripts/` — the per-step bash and Python that gather PR context, build the
  prompt, run Otto, and reconcile the review on the PR.
...
The Python scripts under `scripts/` are dependency-free and unit-testable in
...
1. Edit `action.yaml` or anything under `scripts/`.
