...
- **Sticky summary comment** — the severity tally / summary / reasoning live in one PR issue comment, identified by a hidden `<!-- otto-reviewer:summary -->` marker. It's edited in place (`PATCH`) on each run, or created once (`POST`).
